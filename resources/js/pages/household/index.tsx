...
import { ChevronLeft, ChevronRight, Eye, FilePen, Home, Plus, Trash2, Users } from 'lucide-react';
...
export default function Index({ households }: { households: PaginatedHouseholds }) {
    const [deletingId, setDeletingId] = useState<number | null>(null);

    const handleDelete = (id: number) => {
        toast.warning('Are you sure you want to delete this household record?', {
            action: {
                label: 'Delete',
                onClick: () => {
                    setDeletingId(id);
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                {households.total} total record{households.total !== 1 ? 's' : ''}
                            </p>
                        </div>
                    </div>
                    <Button asChild>
                ...
                <div className="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
                    {households.data.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground">
                            ...
                            <p className="text-sm">Click "Add Household" to create your first record.</p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-sidebar-border/50 bg-muted/40">
                                        ...
                                        <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Father's Name</th>
                                        <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Mother's Name</th>
                                        <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Address</th>
                                        <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Family Income</th>
                                        <th className="px-4 py-3 text-left font-semibold text-muted-foreground">House Status</th>
                                        <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Total Members</th>
                                ...
                                <tbody className="divide-y divide-sidebar-border/30">
                                    {households.data.map((h) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 font-medium">{h.fathers_name}</td>
                                            <td className="px-4 py-3">{h.mothers_name}</td>
                                            <td className="px-4 py-3 max-w-[200px] truncate text-muted-foreground">{h.home_address}</td>
                                            <td className="px-4 py-3 font-mono text-sm">
                                                ₱{Number(h.family_income).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${HOUSE_STATUS_COLORS[h.house_status]}`}>
                                                    {HOUSE_STATUS_LABELS[h.house_status]}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className="inline-flex items-center gap-1 rounded-md bg-muted px-2 py-0.5 text-xs font-semibold text-muted-foreground border border-sidebar-border/30">
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex items-center justify-end gap-1">
                                                    <Button variant="ghost" size="icon" asChild className="size-8">
                                                        <Link href={show({ household: h.id }).url}>
                                                            <Eye className="size-4" />
                                                        </Link>
                                                    </Button>
                                                    <Button variant="ghost" size="icon" asChild className="size-8">
                                                        <Link href={edit({ household: h.id }).url}>
                ...
                {households.total > 0 && (
                    <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="order-2 flex flex-wrap items-center justify-center gap-4 sm:order-1 sm:justify-start">
                            <p className="text-sm text-muted-foreground">
                                {households.from !== null && households.to !== null ? (
                                    <>
                                        Showing{' '}
                                        <span className="font-medium text-foreground">{households.from}</span>
                                        {' – '}
                                        <span className="font-medium text-foreground">{households.to}</span>
                                        {' '}of{' '}
                                        <span className="font-medium text-foreground">{households.total}</span>
                                        {' '}records
                                    </>
                                ) : (
